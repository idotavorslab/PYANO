...
class Elem {
    ...
    css(css) {
        for (let [styleAttr, styleVal] of Dict_1.dict(css).items())
    ...
    cacheAppend(keyChildObj) {
        for (let [key, child] of Dict_1.dict(keyChildObj).items()) {
            this.e.appendChild(child.e);
            this[key] = child;
        }
        return this;
    ...
    cacheChildren(keySelectorObj) {
        for (let [key, selector] of Dict_1.dict(keySelectorObj).items())
    ...
    on(evTypeFnPairs, options) {
        const that = this;
        for (let [evType, evFn] of Dict_1.dict(evTypeFnPairs).items()) {
    ...
    attr(attrValPairs) {
        for (let [attr, val] of Dict_1.dict(attrValPairs).items())
    ...
    async fade(dur, to) {
        ...
        if (opacity === undefined || isNaN(opacity)) {
            console.warn(`fade(${dur}, ${to}) htmlElement has NO opacity at all. recursing`, {
                opacity,
                'this.e.style.display': this.e.style.display,
                'this.e': this.e,
                this: this
            });
            let css = { opacity: Math.abs(1 - to) };
            if (this.e.style.display === 'none')
                css["display"] = '';
            return this.css(css).fade(dur, to);
        }
        else {
