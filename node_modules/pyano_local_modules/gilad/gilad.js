...
/**
 @param {TMessage[]} all_messages
 @param {Truth} truth
 */
async function toMidiFromMessages(all_messages, truth) {
	...
	debugger;
	const firstMsgTime = all_messages[0].time - 0.01;


	for (let [i, msg] of enumerate(all_messages)) {
		if (i == 0)
			continue;

		let note;
		let previousMsg = all_messages[i - 1];
		if (msg.kind == 'on') {
			note = {
...
class Midi extends events.EventEmitter {
	...
	/**
	 @param {TMessage[]} all_messages
	 @param {Truth} truth*/
	async endOnOffTxtStreamsAndWriteMidi(all_messages, truth) {
		console.log(`🎹 [${this.name}] endOnOffTxtStreamsAndWriteMidi(${truth.name})`);
		...
		this._offTxtWriteStream = undefined;
		await toMidiFromMessages(all_messages, truth);
	...
	async newOnOffTxtWriteStreams(truth) {
		console.log(`🎹 [${this.name}] newOnOffTxtWriteStreams(${truth.name})`);
