...
let {
	Alert,
	blockingSwalMixin
} = require("pyano_local_modules/util/Alert");
...
class Midi extends events.EventEmitter {
	...
	constructor({
		...
		this._userPiano.progress(prog => {if(prog==1)console.log(`\tprog: ${prog}`)});
	...
	async endTxtAndWriteMidi(messages, midiFilePath) {
		...
		await this.Convert.toMidiFromMessages(messages, midiFilePath);
