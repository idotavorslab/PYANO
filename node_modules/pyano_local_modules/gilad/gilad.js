...
class Midi extends events.EventEmitter {
	...
	constructor({ name, muteUserPiano = true }) {
		...
		/**@type {Piano}*/
		this._userPiano = new Piano(`file:///${EStore.salamanderDirPath()}`, name).toMaster();
	...
	_playNoteAndMaybeWriteTxt(note, velocity) {
		...
		if (!this._shouldWrite) {
			...
			return console.error(`🎹 _playNoteAndMaybeWriteTxt returning prematurely because !this._shouldWrite`);
		}
	...
	_releaseNoteAndMaybeWriteTxt(note, velocity) {
		if (!this._shouldWrite) {
			Alert.big.blocking({ title: `Something went wrong (!this._shouldWrite)`, text: `Please restart experiment` });
			return console.error(`🎹 _releaseNoteAndMaybeWriteTxt returning prematurely because !this._shouldWrite`);
		}
		let ts = new Date() / 1000;
	...
	_enableWebMidi() {
		const _bindInput = inputDevice => {
			console.log(...small('🎹 _enableWebMidi()._bindInput(inputDevice): '), inputDevice);
			if (!WebMidi.enabled)
				return console.error('🎹 _enableWebMidi()._bindInput(inputDevice), WebMidi.enabled == false, inputDevice:', inputDevice);
			WebMidi.addListener('disconnected', device => {
				Alert.big.warning({
					title: 'Keyboard disconnected! Please turn it on again',
					text: 'Go back to "New Experiment" after connecting'
				});
		...
		if (WebMidi.enabled)
			return console.error('🎹 _enableWebMidi() called from constructor, but was already Webmidi.enabled!');

		WebMidi.enable(err => {
			if (err)
				return console.error('🎹 Webmidi.enable() err!', err);
			...
			WebMidi.addListener('connected', device => {
				...
				console.log(...small(`🎹 WebMidi connected ${deviceType}`));
				if (deviceType == 'input')
