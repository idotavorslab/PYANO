...
let { zip, wait, replace_ext, bool, enumerate, round, max, path_exists, $fadeOut } = require("pyano_local_modules/util");
...
async function toTxtFromMidi(midiFilePath) {
	...
	for (let [noteOn, noteOff] of zip(noteOns, noteOffs)) {


		const on = `${round(ts + noteOn.time, 5)}\tnote=${noteOn.midi}\tvelocity=${round(noteOn.velocity * 127)}\ton`;
		const off = `${round(ts + noteOff.time, 5)}\tnote=${noteOff.midi}\tvelocity=${round(noteOn.velocity * 127)}\toff\n`;
		txtWriteStream.write(`${on}\n${off}`);
	}
...
async function playMidiFile(options) {
	...
	console.log(`playMidiFile(), using playbackPiano: [${playbackPiano.name}]`, {
		midiFilePath,
		animation,
		numOfNotes,
		speed,
	...
	if (!playbackPiano)
		return new Error(`\tpassed no playbackPiano`);
...
class Midi extends events.EventEmitter {
	...
	async newTxtStreamAndMaybeLoadUserPiano(txtFilePath) {
		console.log(`🎹 [${this.name}] newTxtStreamAndMaybeLoadUserPiano(bn: ${Path.basename(txtFilePath)})`);
