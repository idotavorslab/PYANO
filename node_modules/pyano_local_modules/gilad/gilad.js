...
let { concurrent, remove_ext, wait, enumerate, round, max, path_exists, $fadeOut } = require("pyano_local_modules/util");
...
async function toTxtFromMidi(midiFilePath) {
	...
	for (let noteOff of noteOffs) {
		const off = `${round(ts + noteOff.time, 5)}\tnote=${noteOff.midi}\tvelocity=100\toff\n`;
...
async function playMidiFile(options) {
	let { midiFilePath, animation, numOfNotes, speed, mistakes, playbackPiano } = options;
...
class Midi extends events.EventEmitter {
	...
	constructor({ name, muteUserPiano = true }) {
		...
		let volume = muteUserPiano
		             ? -Infinity
		             : 0;
		console.log(`🎹 [${this.name}]`, { volume, name, muteUserPiano });
	...
	async endTxtAndWriteMidi(messages, midiFilePath) {
		console.log(`🎹 [${this.name}] endTxtAndWriteMidi(bn: ${Path.basename(midiFilePath)})`);
	 ...
	 */
	async newTxtWriteStream(txtFilePath) {
		console.log(`🎹 [${this.name}] newTxtWriteStream(bn: ${Path.basename(txtFilePath)})`);

		// check and maybe error
		(() => {
			if (this._shouldWrite) {
				console.error("newTxtWriteStream() was called but this._shouldWrite was already true");
			}
			if (this._txtWriteStream != undefined) {
				console.error("newTxtWriteStream() was called but this._txtWriteStream was NOT undefined (should have been)");
			}

			if (!txtFilePath.endsWith('.txt')) {
				console.error(`newTxtWriteStream(), txtFilePath doesnt end with .txt. txtFilePath: ${txtFilePath}`);
			}
			if (txtFilePath.lastIndexOf('.txt') != txtFilePath.indexOf('.txt')) {
				console.error(`newTxtWriteStream(), txtFilePath has multiple .txt: ${txtFilePath}`);
			}
			if (!WebMidi.enabled)
				console.error(`newTxtWriteStream(), WebMidi.enabled == false`);
		...
		if (exists) {
			throw new Error(`newTxtWriteStream() txtFilePath exists. ${txtFilePath}`);
		}
	...
	_playNoteAndMaybeWriteTxt(note, velocity) {
		...
		if (this._shouldWrite) {
			let ts = new Date() / 1000;
			this._txtWriteStream.write(`${round(ts, 5)}\tnote=${note}\tvelocity=${round(velocity * 127)}\ton\n`);
	...
	_releaseNoteAndMaybeWriteTxt(note, velocity) {
		...
		if (this._shouldWrite) {
			let ts = new Date() / 1000;
			this._txtWriteStream.write(`${round(ts, 5)}\tnote=${note}\tvelocity=${round(velocity * 127)}\toff\n`);
	...
	/*async playMidiFile({ midiFilePath, animation, numOfNotes, speed, mistakes, playbackPiano }) {
		console.log(`🎹 [${this.name}] playMidiFile(), using playbackPiano: [${playbackPiano.name}]`, {
			midiFilePath,
			animation,
			numOfNotes,
			speed,
		...
		if (!playbackPiano._loaded) {
			console.log(`\t🎹 [${this.name}].playMidiFile() passed playbackPiano was NOT loaded (shouldn't happen)`);
			Alert.small.info('Loading piano', 'might take a few seconds...');
			await playbackPiano.load();
			console.log(`\t🎹 [${this.name}].playMidiFile() done loading passed (unloaded) playbackPiano (shouldn't happen)`);
		}
		let midiRead = await MidiConvert.load(midiFilePath);
