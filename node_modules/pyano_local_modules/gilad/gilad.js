...
async function playMidiFile(options) {
	let { truth, animation, numOfNotes, playbackRate, mistakes, animationPiano } = options;
		...
		const scheduleNoteOn = (onTime, onEvent) => {
			...
			if (mistakes) {
				...
				if (bool(mistakes[index])) {
					fill = mistakes[index] == 'accuracy' ? 'red' : 'blue';
				}
			}
...
function _sliceNotes(readTrack, numOfNotes, playbackRate) {
	...
	console.log(...small('\t_sliceNotes()'), { noteOns, noteOffs });
	if (playbackRate) {
