...
class Piano extends Tone.AudioNode {
	...
	async load(url = this._localSamplesPath) {
		...
		console.log(`🎹 [${this.name}] %cPiano.js loaded`, 'color:#278b3e; font-size:10px');
		this._loaded = true;
		return true;
	}
	...
	pedalDown(time = Tone.now()) {
		if (this.loaded) {
			if (this.toSeconds(time) != time) {
				console.error(`pedalDown this.toSeconds(time)!=time. this.toSeconds(time): ${this.toSeconds(time)}, time: ${time}`);
				return this
			}
	...
	pedalUp(time = Tone.now()) {
		if (this.loaded) {
			if (this.toSeconds(time) != time) {
				console.error(`pedalUp this.toSeconds(time)!=time. this.toSeconds(time): ${this.toSeconds(time)}, time: ${time}`);
				return this
			}
			if (this._pedal.isDown(time)) {
				this._pedal.up(time);
	...
	keyDown(note, time = Tone.now(), velocity = 0.8) {
		...
		if (this.toSeconds(time) != time) {
			console.error(`keyDown this.toSeconds(time)!=time. this.toSeconds(time): ${this.toSeconds(time)}, time: ${time}`);
			return this
		}
	...
	keyUp(note, time = Tone.now(), velocity = 0.8) {
		...
		if (this.toSeconds(time) != time) {
			console.error(`keyUp this.toSeconds(time)!=time. this.toSeconds(time): ${this.toSeconds(time)}, time: ${time}`);
			return this;
		}
		if (Tone.isString(note)) {
			note = Math.round(Tone.Frequency(note).toMidi());
