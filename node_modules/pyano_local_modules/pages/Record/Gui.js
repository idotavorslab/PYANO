...
let { bool, remove_ext, underscore2space } = require("pyano_local_modules/util");
...
function animateRecordBtn({ on }) {
	...
	// if (on)
	// 	$recordBtn.addClass(animationClass);
	// else // off
	// 	$recordBtn.removeClass(animationClass);
...
let _$truthFiles = [...new Set(truthFilesNoExt)]
...
let videoProcessDiv = $('<div id="video_process_div">')
	.append(
		'<p id="video_process_title">Video Processing</p>',
		$('<button id="video_file_browse_btn">')
			.text('Trim a raw mp4 file...')
			.addClass('my-btn active-btn'),
		$('<button id="video_file_subvideos_btn">')
			.text(`Trim ${EStore.truthFileName({ withExtension: false })} to sub-videos`)
			.addClass('my-btn active-btn')
	);
...
function repopulateTruthFiles() {
	let truthFilesNoExt = fs.readdirSync(truthsDirPath).map(remove_ext);
	_$truthFiles = [...new Set(truthFilesNoExt)]
		.filter(f => truthFilesNoExt.count(f) == 2)
		.map(f =>
			$(`<div id="${f}">${f}</div>`)
				.css({
...
module.exports = {
	...
	$inputDiv, $recordPath, videoProcessDiv, setEditableSpanText, repopulateTruthFiles,
