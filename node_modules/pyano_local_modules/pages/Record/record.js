...
let Gui = require("./Gui");
let { span, $fadeInMany, $fadeOutMany, $fadeOut, $fadeIn, underscore2space, round } = require('pyano_local_modules/util');
...
async function doVideoStuff() {
	...
	if (vidFilePath == null)
		return;
	const vidFileName = path.basename(vidFilePath);
	const vidNewPath = path.join(EStore.truthsDirPath(), vidFileName);
	if (vidNewPath != vidFilePath) {
		let { value: shouldMove } = await Alert.small.info(`Move ${vidFileName} to truths folder?`, null, true);
		console.log({ shouldMove });
		await fs.renameSync(vidFilePath, vidNewPath);
		Alert.small.info('Successfully moved', null);
		vidFilePath = vidNewPath;
	}
