...
async function _startRecord(recordMidi) {
	...
	try {
		await recordMidi.newOnOffTxtWriteStreams(new Truth(_getRecordPath()));
...
async function _stopRecord(recordMidi, playbackPiano) {
	...
	const truth = new Truth(_getRecordPath());
	const fs = require("fs");
	...
	let { all_msgs, on_msgs, off_msgs } = msgs[0];
	await recordMidi.endOnOffTxtStreamsAndWriteMidi(all_msgs, truth);
...
/**@param {?string} ext*/
function _getRecordPath(ext = null) {
		...
		Gui.$recordBtn.click(async () => {
			if (!alreadyRecording) {
				...
				try {
					...
					console.warn(e);
					Alert.small.error('Stopping record failed', e);
				}
			}
