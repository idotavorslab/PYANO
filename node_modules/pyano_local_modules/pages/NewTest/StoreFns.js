...
function setNewTruth(file) {
	...
	if (Path.dirname(file) != EStore.truthsDirPath())
		return Alert.small.warning(`File "${fsx.basename(file)}" isn't in truths dir`, `Place it here: ${EStore.truthsDirPath()}`);
	const truth = new Truth(fsx.remove_ext(file));
	if (truth instanceof Error)
		return Alert.small.warning(truth.message);
