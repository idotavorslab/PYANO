...
function saveAsCurrentTest() {
	console.log(...small(`NewTest.StoreFns.saveAsCurrentTest()`));
...
function saveConfigsAs() {
	console.log(...small(`NewTest.StoreFns.saveConfigsAs()`));
	const dialogOptions = {
	...
	remote.dialog.showSaveDialog(getCurrentWindow(), dialogOptions, (filename, bookmark) => {
		console.log('saveConfigsAs()', { filename, bookmark });
...
function loadConfig() {
	...
	remote.dialog.showOpenDialog(getCurrentWindow(), dialogOptions, async files => {
		...
		try {
			...
			// TODO: validate data
			const config = EStore.config();
			for (let [key, value] of data.items()) {
				try {
					config[key] = value;
				} catch (e) {
					console.warn(`tried setting config[${key}] = ${value}, failed. ignoring. error message:\n${e.message}`);
				}
			}
