...
/**@param {String} file*/
function setNewTruthFile(file) {
	console.log(...small(`NewTest.StoreFns.setNewTruthFile(${file})`));
	const relativePath = Path.relative(EStore.truthsDirPath(), file);
	console.log(...small(`\trelativePath = ${relativePath}`));
	const relativePathNoExt = remove_ext(relativePath);
	EStore.setTruthFilePath(relativePathNoExt)
	      .then(truthFilePath => {
		      console.log(`Set new truth file path: ${truthFilePath}`);
		      const truthFileName = Path.basename(relativePathNoExt);
		      Alert.small.success("Set new truth file", truthFileName);
		      Gui.setTruthFileSubtitle(truthFileName, EStore.truthFileTotalNotes());
		      LevelsPart().toMode(EStore.currentTest().learning_type);
	      })
	      .catch(e => Alert.small.error('Failed setting truth file', e.message));
}

function browseAndSetNewTruthFile() {
	console.group('browseAndSetNewTruthFile()');
