	...
	const { value: subject } = await Alert.big.blocking({
		...
		onOpen: htmlElement => {
			...
			inputDiv.onkeydown = kbevt => {
				let justPressed = kbevt.key;
				if (justPressed.lower() === 'tab') {
					// const value = editableSpan.innerText + autocompleteSpan.innerText;
			...
			editableSpan.oninput = async function (inputevt) {
				...
				// console.log(inputevt, { justTyped, subjectInput });
				if (/[a-zA-Z0-9_ \-]/.test(justTyped)) {
					...
					// this.innerText = subjectInput.substr(0, subjectInput.length - 1);
