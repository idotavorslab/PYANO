...
async function _startTestIfReady() {
	let levelsDicts = LevelsPart().getLevelsArr().toTLevels();
	EStore.currentConfig().levels = levelsDicts;
	debugger;
	// EStore.set(`current_test.levels`, levelsDicts);
	// StoreFns().saveAsCurrentTest();
	if (!bool(EStore.subjectName()))
...
const _currentSubject = EStore.currentConfig().current_subject;
const $currentSubjectSubtitle = $('<div id="page_subtitle_current_subject">')
	.html(bool(_currentSubject)
	      ? `Current subject: ${strong(_currentSubject)}`
	      : 'Current subject: --');
...
let $readyBtn = $('<button id="page_ready_btn">')
	.html('Ready')
	.click(async () => await _startTestIfReady());

let $saveBtn = $('<button id="page_save_btn">')
	.html("Save")
	.click(async () => {
		$saveBtn.toggleClass('inactive-btn', true)
		        .toggleClass('active-btn', false);
		await StoreFns().saveAsCurrentTest();
		$saveBtn.toggleClass('inactive-btn', false)
