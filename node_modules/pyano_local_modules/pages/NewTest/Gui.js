...
async function _startTestIfReady() {
	...
	const { value: subject } = await Alert.big.blocking({
		...
		onOpen: htmlElement => {
			...
			autocompleteSpan.innerText = 'Letters, digits, spaces, under_scores or-dashes. Tab completes, Esc uncompletes.';
			...
			inputDiv.onkeydown = (kbevt) => {
				...
				const shouldComplete = kbevt.key.lower() === 'tab' && (bool(editableSpan.innerText) || subjects.includes(autocompleteSpan.innerText));
				if (shouldComplete) {
					editableSpan.innerText += autocompleteSpan.innerText;
					autocompleteSpan.innerText = '';
				} else if (kbevt.key.lower() === 'escape') {
					autocompleteSpan.innerText = '';
				}
			};
			editableSpan.oninput = async function (inputevt) {
				...
				if (/[a-zA-Z0-9_ \-]/.test(justTyped)) {
					...
					this.innerText = subjectInput.removeAll(justTyped);
