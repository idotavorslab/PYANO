...
function setSavePathSubtitle(savePath) {
	$savePathSubtitle.html(savePath);
}
...
const $savePathSubtitle = $(`<div>${EStore.config().save_path}</div>`);
...
const $loadConfigBtn = $('<button id="page_load_config_btn">')
	...
	.click(async () => {
		const storeFns = StoreFns();
		/*if ($saveBtn.isActive()) {
			const answer = await Alert.big.blocking({
				title: 'Save changes before loading new config?',
				text: `Will be saved to: ${EStore.save_path}`
			});
			if (answer.value)
				storeFns.saveExperiment();
			else
				Alert.small.info(`Didn't save`);
		}
		*/
		storeFns.loadExperiment();
	});
