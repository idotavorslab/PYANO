...
let { path_exists, strong, bool } = require("pyano_local_modules/util");
...
async function startTestIfReady() {
	...
	if (EStore.subjectName() == null) {
		await Alert.small.warning("Please specify current subject.");
		return;
	}
	let truthNoExt = EStore.truthFileName({ withExtension: false });
	/*if (EStore.isDemoVideo()) {
		let mp4Exists = await path_exists(EStore.truthFilePath('mp4'));
		let movExists = await path_exists(EStore.truthFilePath('mov'));
		let onsetsExists = await path_exists(`${EStore.truthFilePath()}_onsets.json`);
		if (!mp4Exists) {
			await Alert.small.warning(`No mp4 video file for current truth file (${truthNoExt})`, "Use File Tools Page.");
			return;
		}
		if (!onsetsExists) {
			await Alert.small.warning(`No onsets file for current truth file (${truthNoExt})`, "Use File Tools Page.");
			return;
		}
	}
	*/
	if (!await path_exists(EStore.truthFilePath('txt'))) {
		await Alert.small.warning(`Couldn't find truth file: (${truthNoExt})`, "Use File Tools Page.");
		return;
	}
	/*if (!await path_exists(EStore.truthFilePath('mid'))) {
		await Alert.small.warning(`No matching midi file for current truth file (${truthNoExt})`, "Use File Tools Page.");
		return;
	}
	*/
...
const $currentTruthFileSubtitle = $('<div>Truth File:</div>')
	...
	.click(() => StoreFns().browseAndSetNewTruthFile())
	.css('cursor', 'pointer');
