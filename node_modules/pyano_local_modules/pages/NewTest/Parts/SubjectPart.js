...
async function onSubmitSubjectClick() {
	...
	if (ok)
		PyFns().check_create_new_subject(subjectIdInputVal);
	reset({ inputMissing: false });
	const config = EStore.config();
	config.current_subject = subjectIdInputVal;
	// EStore.set('current_test.current_subject', subjectIdInputVal);
	// EStore.update('subjects', subjectIdInputVal);
	Gui.setCurrentSubjectSubtitle(subjectIdInputVal);
	// const subjectList = config.getSubjectDirNames();
	// console.log('onSubmitSubjectClick, setting subjects:', subjectList);
	// EStore.subjects = subjectList;
	/*EStore.set('subjects', subjectList);
	if (!(subjectList.includes(config.current_subject)))
...
let $submitSubjectBtn = $('<button id="subject_submit_subject_btn">')
	...
	.html('Submit')
	.click(async () => await onSubmitSubjectClick());
