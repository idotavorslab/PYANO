...
/*const $ = elem => {
	...
	$elem.touchstart = fn => {
		debugger;
		const ret = $elem.on('touchstart', ev => {
			debugger;
			ev.preventDefault();
			fn();
			return $elem;
...
const LevelsPart = (() => {
	...
	class $Level {
		...
		constructor(row, levelIndex, right = 0) {
			...
			this.$accuracySwitch = $('<button class="my-btn toggle-btn-off">')
				.css({
					gridColumn: `${baseGridColumn + 2}/${baseGridColumn + 2}`,
					gridRow: this.gridRow
				})
				.click(() => this.$accuracySwitch
			...
			this.$trialsSelect = $('<select class="my-select">')
				.css({
					gridColumn: `${baseGridColumn + 5}/${baseGridColumn + 5}`,
					gridRow: this.gridRow
				});


			let $selects = this.$trialsSelect.add(this.$notesSelect);
			for (let i of range(0, EStore.maxNotesOfLevels()))
		...
		$elements() {
			return [this.$i, this.$notesSelect,
			        this.$accuracySwitch, this.$rhythmSwitch,
	...
	let $subtitlesContainer = $('<div id="levels_subtitles_container">')
		...
		.append(
			...
			'<div id="levels_accuracy_subtitle">ACCURACY',
