...
const LevelsPart = (() => {
	...
	class $Level {
		...
		constructor(row, levelIndex, right = 0) {
			...
			this.$rhythmSwitch = $('<button class="my-btn toggle-btn-off">', "toggle")
				.css({
			...
			this.$tempoSelect = $('<select disabled class="my-select my-select-disabled">', "select")
				.css({
			...
			let $notesAndTrials = this.$trialsSelect.add(this.$notesSelect);
			for (let i of range(0, EStore.truth().numOfNotes()))
				$notesAndTrials.append(`<option value="${i}">${i}`);


			$notesAndTrials.change(() => {
