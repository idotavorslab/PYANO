...
/**@typedef {'demo_type' | 'errors_playingspeed' | 'allowed_rhythm_deviation' | 'learning_type'} TFieldName*/
...
class ToggleSetting {
	...
	_setStoreAndToggleGui(toFirst) {
		...
		let fieldValue = toFirst ? this.firstOpt.storeVal : this.secondOpt.storeVal;
	 ...
	 */
	jQueries(options) {
		let withLabel = options ? options.withLabel : true;

		let jQs = this.firstOpt.$jQ.add(this.secondOpt.$jQ);
		if (withLabel)
...
class SelectSetting {
	...
	_popuplateOptionsByStore({ rangeStop, valueFn }) {
		let currentTest = EStore.get('current_test');
		let valueInCfg = this._valFromString(currentTest[this.fieldName]);
...
let SettingsPart = (() => {
	...
	function toMode(mode) {
		...
		errorsPlayingSpeed.toggle(mode == "accuracy");
		allowedRhythmDeviation.toggle(mode == "tempo");
	}

	let $Div = $('<div id="settings_div">');
	let demoType = new ToggleSetting({
		fieldName: 'demo_type',
		elements: {
	...
	let errorsPlayingSpeed = new FloatSelectSetting({
	...
	let allowedRhythmDeviation = new PrecentSelectSetting({
	...
	/*let learningType = new ToggleSetting({
