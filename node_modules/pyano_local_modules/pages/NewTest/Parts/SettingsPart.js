...
let { bool, range, underscore2space, float } = require("pyano_local_modules/util");
...
class Setting {
	...
	_setStoreAndToggleGui(toFirst) {
		...
		Alert.small.success(`${underscore2space(this.fieldName).title()}`, `set to ${fieldValue}`);
...
class SelectSetting {
	...
	_setNewValueInStore() {
		...
		// console.log({ '_setNewValueInStore, this._previousValue': this._previousValue });
		EStore.set(`current_test.${this.fieldName}`, fieldValue);
		let msg = `${underscore2space(this.fieldName).title()} set to ${fieldValue}`;
		if (!this.isPercent)
			msg += 'x';
