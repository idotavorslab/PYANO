...
let { replace_to_ext, zip, round, path_exists, remove_ext, $fadeOut, $fadeInMany, $fadeOutMany, $fadeIn } = require("pyano_local_modules/util");
...
async function handleMov(name, path) {
	let mp4 = replace_to_ext(path, 'mp4');
	let { value: shouldConvert } = await Alert.big.blocking({
		title: 'Convert to mp4?',
		html: `${name} => ${Path.basename(mp4)}`,
...
const MP4 = (() => {
	async function createSubVersions(trimmedName, path) {
		console.log(...log(`createSubVersions(trimmedName,path)`), { trimmedName, path });
		let match = EStore.truthFilesList('txt')
		                  .find(f => remove_ext(f) == trimmedName.upTo('_trimmed'));
		if (!match) {

			Alert.small.error(`Didn't find a matching truth file`, `Tried to match: ${trimmedName.upTo('_trimmed')}`);
			return;
		}
		console.log('\t', { match });
		let shouldContinue = await Alert.big.blocking({
			title: `Found a matching truth file`,
			html: `Video will be cut based on <b>${match}</b>`,
			showConfirmButton: true,
			showCancelButton: true
		});
		...
		// let trimmedFilePath = `${truthFilePathNoExt}_trimmed.mp4`;
		// console.log(...log(`\tcreateSubVersions`), { notes, truthFilePathNoExt, trimmedFilePath });
		let messages = await Python.runAsync('InsideTest/normalize_txt_file.py', {
			args: [Path.join(EStore.truthsDirPath(), match)],
			mode: "json"
		});
		...
		for (let [noteCount, vidDur] of zip(notes, vidsDurations)) {
			try {
				let trimmedPath = await trimAndForceSilence(path, 0, silenceLen, vidDur, noteCount);
		...
		Alert.small.success(title, `of file: ${trimmedName}`);
	}

	async function cleanSilence(name, path) {
		console.log(...log(`cleanSilence(name = ${name}, path)`));
		...
		let { value: shouldClean } = await Alert.big.blocking({
			...
			html: `${name} => ${Path.basename(trimmedFilePath)}`,
			showConfirmButton: true,
			showCancelButton: true,
		});
		if (!shouldClean) {
		...
		Alert.big.blocking({ title: 'How long was it until you played the first note?' },
			{
				...
				clickFn: async $s => {
					...
					await trimAndForceSilence(path, secs, EStore.vidSilenceLen());
...
async function onDrop(e) {
	...
	if ("text".in(type)) {
		...
		if (await path_exists(replace_to_ext(path, 'mid'))) {
		...
		try {
			await toMidiFromMessages(messages, replace_to_ext(path, 'mid'));
			Alert.small.success('Converting from txt to midi succeeded.');
		} catch (e) {
