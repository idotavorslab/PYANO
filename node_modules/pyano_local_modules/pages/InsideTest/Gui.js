...
async function playPreTrialDemo({ animationPiano, levels, truth }) {
	...
	if (isFirstTrial)
		...
		$bigMessage.text(`Let's move on to the ${currentLevel.internalTrialIndex.human()} trial of the ${currentLevel.index.human()} level.`);
	if (isFirstTrial)
	...
	let tempo = levels.getNextTempoOfThisNotes();
	/*if (!currentLevel.rhythm) {
		tempo = levels.getNextTempoOfThisNotes();
		if (tempo == null) // all levels with this number of notes have no tempo
			tempo = 100;
		console.log(...log(`\tgot next tempo of this notes category, tempo = ${tempo}`, { sm: true }));
	} else {
	...
	*/
	...
	const _smallsec = [`Here's ${shouldAlsoRunVideo ? "a video" : "an animation"}`,
	                   `showing only these <b>${currentLevel.notes}</b> notes.<br>`];
	if (currentLevel.rhythm)
		_smallsec.push("<b>Try to keep rhythm. Don't play any slower than the following demo.</b>");
	else
	...
	if (shouldAlsoRunVideo) {
		...
		$smallMessage.html(`You'll now watch the same <b>${currentLevel.notes}</b> notes played with an animation.`);
...
async function showFailedTrialFeedback(animationPiano, levels, doneTrialResult, truth) {
	...
	if (advance_trial) {
		$smallMessage.text(`You almost got the ${levels.current.internalTrialIndex.human()} trial right.`);
		...
		const _smallsec = ["Please try this trial again. The animation will play, showing the notes you got wrong."];
		if (tempo_str && tempo_str != 'ok')
			_smallsec.push(`<br>Additionally, you played too ${tempo_str}.`);
	...
	if (levels.current.rhythm)
		playOptions.playbackRate = levels.getNextTempoOfThisNotes() / 100;
	else // TODO: should slow down only wrong notes
		playOptions.playbackRate = EStore.currentTest().errors_playingspeed;
...
async function showPassedTrialFeedback(animationPiano, levels, doneTrialResult, truth) {
	...
	$smallMessage.html(`Let's show you those same <b>${currentLevel.notes}</b> notes you just got correctly to consolidate the memory.`);
