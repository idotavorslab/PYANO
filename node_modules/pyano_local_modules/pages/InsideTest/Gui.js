...
class Video {
	...
	_promiseMousedown() {
		console.log(...logsm('_promiseMousedown() started'));
		return new Promise(resolve =>
			this.vidElm.addEventListener('mousedown', e => {
				console.log('%cvideo mousedown', 'color:#de7d36', { isTrusted: e.isTrusted });
				console.log(...logsm('_promiseMousedown() resolving'));
				resolve();
			}, { once: true }));
	}
	...
	_logEvents() {
		...
		for (let [evprop, val] of enumerate(this))
			if (evprop.endsWith('Fired')) {
				const strings = logsm(evprop.upTo('Fired'));
				if (val)
...
async function _setup({ includeSecondary = true, fadeInBigMsg = true }) {
	...
	await wait(1500 * F);
	if (includeSecondary)
		await $fadeOutMany(1500 * F, $smallMessage, $bigMessage, $smallMessageSecondary);
	else
		await $fadeOutMany(1500 * F, $smallMessage, $bigMessage);
...
async function playPreTrialDemo(midi, playbackPiano, levelIndex, trialIndex, numOfNotes) {
	...
	*/
	...
	if (EStore.isDemoVideo() && trialIndex == 0 && !EStore.isLearningTempo()) {
	...
	*/
	...
	await $fadeInMany(300, $bigButton, $smallMessage, $Sidebar);
