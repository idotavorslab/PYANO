...
async function playLevelIntroDemo({ animationPiano, levels, truth }) {
	...
	*/
	...
	await _$messagesInAndOut({ fadeSecondaryInOut: true, long: true });
...
async function showFailedTrialFeedback(animationPiano, levels, doneTrialResult, truth) {
	...
	let _msgsInOutOpts = { fadeInBigMsg: false, fadeSecondaryInOut: false, long: true };
	...
	if (doneTrialResult.hasAccuracyAndRhythmMistakes()) {// if has both rhythm and acc mistakes, show only acc mistakes (nullify rhythm mistakes)
		doneTrialResult.mistakes = doneTrialResult.mistakes.map(m => m == 'rhythm' ? null : m);
