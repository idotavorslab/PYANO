...
async function _$bigMessageInGuiOut() {
	console.log(...logmod('\t_$bigMessageInGuiOut()'));
	await concurrent(
		$fadeIn($bigMessage, 700 * F),
		$fadeOutMany(200 * F, $bigButton, $smallMessage, $Sidebar, $pageSubtitle)
	);
	await wait(700 * F);
}
...
/**@param {Midi} midi
 ...
 @param {Truth} truth
 @param {number} maxNotesOfLevels*/
async function playWholeTruthDemo(midi, playbackPiano, truth, maxNotesOfLevels) {
	...
	if (shouldRunVideo)
		await _runVideo(maxNotesOfLevels);
	else
		await _runAnimation(midi, {
			truth,
			animation,
			numOfNotes: maxNotesOfLevels,
			playbackPiano
		});
...
async function playPreTrialDemo(options) {
	...
	if (trialIndex == 0)
		$bigMessage.text(`${levelIndex.human(true).title()} level, ${trialIndex.human(true)} trial.`);
	else
		$bigMessage.text(`Let's move on to the ${trialIndex.human(true)} trial.`);
...
async function showTestCompleteMessages() {
	...
	await $fadeInMany(700, $bigMessage, $smallMessage, $smallMessageSecondary, $Sidebar);
}
