...
let {
	concurrent, $fadeOut, $fadeIn, int, wait, num2eng, strong, range, $fadeOutMany, $fadeInMany, bool, Alert
...
class Video {
	...
	_promiseLoadedData() {
		return new Promise(resolve => this.vidElm.onloadeddata = () => resolve());
	}

	_promiseEnded() {
		return new Promise(resolve => this.vidElm.onended = () => resolve());
	}
	...
	/**@param {Number} numOfNotes
	 @return {Promise<void>}*/
	async play(numOfNotes = -1) {
		console.log(`Video.play(numOfNotes = ${numOfNotes})`);
		if (numOfNotes != -1) {
			let trimmedFilePath;
			console.log('duration before changing src', this.vidElm.duration);
			trimmedFilePath = `${EStore.truthFilePath()}_trimmed_${numOfNotes}N.mp4`;
			let vidsrcfile = `file:///${trimmedFilePath}`;
			console.log({ vidsrcfile });
			this.vidElm.src = vidsrcfile;
			this.vidElm.load();
		}
		...
		try {
			await concurrent(this.vidElm.play(), wait(this.vidElm.duration * 1000));
		} catch (e) {
			console.warn(e);
