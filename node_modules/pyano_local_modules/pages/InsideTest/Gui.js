...
class Animation {
	...
	/*async playTruth() {
		console.log('animation.playTruth()');
		await this._play(null, { applySpeedSetting: false });
	}

	async playPreTrial(numberOfNotes) {
		console.log(`animation.playPreTrial(numberOfNotes = ${numberOfNotes})`);
		await this._play(null, { applySpeedSetting: false, numberOfNotes });
	}

	async playErrors(mistakes) {
		console.log(`animation.playErrors(mistakes.length = ${mistakes.length})`);
		await this._play(mistakes, { applySpeedSetting: true });
	}
	*/

	/**
	 If mistakes is null, function knows it's a tutorial.
	 This means all notes of truth file will be played, and only in green.
	 Otherwise, only
	 @param {String[]} mistakes
	 @param {Boolean} applySpeedSetting
	 @param {Number} NumberOfNotes
	 */
	/*	async _play(mistakes, { applySpeedSetting, numberOfNotes = -1 }) {
			console.log('animation._play()');
			await wait(500);
			let shouldPlayWholeFile = mistakes == null;
			return new Promise(async resolve => {

				this.$element.show();
				let truthSliced;
				if (this.truthMessages == undefined) {
					this.truthMessages = await PyFns.extractMessagesFromTruthFile();
				}
				// let truthMessages = await PyFns.extractMessagesFromTruthFile();
				if (shouldPlayWholeFile) {
					if (numberOfNotes != -1) {
						truthSliced = this.truthMessages.slice(0, numberOfNotes);
					} else {
						truthSliced = this.truthMessages;
					}
				} else { // mistakes has value
					truthSliced = this.truthMessages.slice(0, mistakes.length);
				}

				const errorsPlayingSpeed = EStore.get('current_test.errors_playingspeed');
				const slowdownFactor = 1 / errorsPlayingSpeed;
				for (let [i, msg] of enumerate(truthSliced)) {
					let noteDuration;
					try {
						noteDuration = this.truthMessages[i + 1].time_delta * 1000;
						if (applySpeedSetting)
							noteDuration *= slowdownFactor;

					} catch (e) {
						noteDuration = 1000;
					}
					if (shouldPlayWholeFile)
						await this.markKey(msg.note, noteDuration, 'green');
					else
						await this.markKey(msg.note, noteDuration, bool(mistakes[i])
																				 ? 'red'
																				 : 'green');
				}
...
async function playPreTrialDemo(midi, playbackPiano, levelIndex, trialIndex, numOfNotes) {
	...
	await $fadeIn($smallMessageSecondary, 1000);
	await wait(2000);
...
async function showFailedTrialFeedback(midi, playbackPiano, trialIndex, mistakes) {
	...
	await concurrent($fadeIn($bigMessage, 1000), $fadeOutMany(300, $smallMessage, $bigButton));
	await wait(1000);
	// await $fadeOutMany(300, $smallMessage, $bigButton);
	// await $fadeIn($bigMessage, 1000);
...
async function showTestCompleteMessages() {
	...
	await $fadeInMany(1000, $bigMessage, $smallMessage);
}
