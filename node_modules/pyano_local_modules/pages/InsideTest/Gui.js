...
class Video {
    ...
    async init(truth) {
        ...
        let data = fs.readFileSync(truth.onsets.path);
        ...
        console.log(`📽 Video.data from onsets.json | this.firstOnset: ${this.firstOnset}, this.lastOnset: ${this.lastOnset}, data:`, {data});
    ...
    _setEventHooks() {
        ...
        const events = {
            abort: {
                description: `Resource was not fully loaded, but not as the result of an error:\npossibly if the element is already in the process of loading media when .load() was called`,
                loglevel: "warn"
            },
            canplay: {
                description: "Can play but not enough has been loaded to play the media up to its end",
                loglevel: "info"
            },
            canplaythrough: {
                description: "Can play the media up to its end",
                loglevel: "good"
            },
            durationchange: {
                // duration is readonly
                description: "The duration attribute has been updated.",
                loglevel: "warn"
            },
            emptied: {
                description: "The element has already started / finished loading, and was emptied because .load() was called",
                loglevel: "warn"
            },
            ended: {
                description: "Reached end",
                loglevel: "info"
            },
            error: {
                description: "Resource could not be loaded due to an error.",
                loglevel: "error"
            },
            loadeddata: {
                description: "First frame of media has finished loading",
                loglevel: "good"
            },
            loadedmetadata: {
                description: "Loaded meta data",
                loglevel: "good"
            },
            loadstart: {
                description: "Fired when the browser has started to load",
                loglevel: "good"
            },
            pause: {
            ...
            stalled: {
                description: "Trying to fetch media data, but is not coming",
                loglevel: "error"
            },
            suspend: {
            ...
            timeupdate: {
                description: "currentTime was set",
                loglevel: "warn"
            },
            waiting: {
                description: "Stopped because of a temporary lack of data",
                loglevel: "warn"
            },
        };

        for (const [evname, {description, loglevel}] of Object.entries(events)) {
            const evFired = `${evname}Fired`;
            this[evFired] = false;
            this.vidElm[`on${evname}`] = () => {
                this[evFired] = true;
                if (loglevel == 'good') {
                    console.log(`%c${evname} fired: ${description}`, 'color:#00c100')
                } else {
                    console[loglevel](`${evname} fired: ${description}`)
                }
            };
        }
    }
    ...
    setCurrentTimeToFirstOnset() {
        console.log(`📽 setCurrentTimeToFirstOnset() | this.firstOnset: `, this.firstOnset,
            `setting to: `, this.firstOnset - 0.1,
            `currentTime before setting: ${this.vidElm.currentTime}`
        );
        this.vidElm.currentTime = this.firstOnset - 0.1;
    }

    /// Called by play() after await this._getWaitValue(numOfNotes, playbackRate);
    ...
    /// Called by play() only if !canplaythroughFired || !loadeddataFired
    /**
     * @returns {Promise<unknown>}
     */
    _promiseloadeddata() {
        console.log('\t📽 _promiseloadeddata() started');
        // debugger; // check with HAVE_DATA enum
     ...
     * @returns {Promise<unknown>}
     */
    async _promiseFadeIn() {
        console.log(...small('\t📽 _promiseFadeIn()'));
        if (this.vidElm.volume != 0) {
            console.warn('📽 _promiseFadeIn() | volume is not 0, it is: ', this.vidElm.volume);
        }
        return new Promise(resolve => this.$element.animate({volume: 1}, 100, 'linear', resolve));
        await asx.wait(100);
    }
     ...
     * @returns {Promise<unknown>}
     */
    async _promiseFadeOut(playbackRate) {
        console.log(...small('\t📽 _promiseFadeOut()'));
        if (this.vidElm.volume != 1) {
            console.warn('📽 _promiseFadeOut() | volume is not 1, it is: ', this.vidElm.volume);
        }
        const ms = Math.round(50 / playbackRate);
        this.$element.animate({volume: 0}, ms, 'linear');
        await asx.wait(ms);
    }

    /**@param {number} numOfNotes
     @param {number} playbackRate*/
    _getWaitValue(numOfNotes, playbackRate) {
        // debugger;
        let waitValue;
        if (numOfNotes == null) {
            console.error("_getWaitValue got numOfNotes == null");
            if (this.lastOnset < this.vidElm.duration) {
                let diff = this.vidElm.duration - this.lastOnset;
                diff = Math.min(1.5, diff);
            ...
            // Computes the time including the duration of the last note, by
            // using the n+1 note's ON time.
        ...
        console.log(`\t 📽 _getWaitValue(numOfNotes = ${numOfNotes}, playbackRate = ${playbackRate}) → waitValue`, {
            waitValue,
    ...
    async play(numOfNotes, playbackRate) {
        ...
        if (playbackRate <= 0 || playbackRate > 3) {
            console.error(`Video.play() got playbackRate le 0 or gt 3: ${playbackRate}`);
        ...
        if (!canplaythroughFired || !loadeddataFired) {
            console.warn('\t!canplaythroughFired || !loadeddataFired, calling video.load(), awaiting _promiseloadeddata() plus 500ms');
        ...
        const waitValue = this._getWaitValue(numOfNotes, playbackRate);
        await this._fakeMousedown();
        await this._promiseFadeIn();
        console.log(`📽 concurrent: vide.play(), wait(waitValue =`, waitValue);
        await Promise.all([this.vidElm.play(), asx.wait(waitValue)]);
        await this._promiseFadeOut(playbackRate);
        this.vidElm.pause();
        console.groupEnd();
    }
    ...
    constructor($animation) {
        ...
        for (const i of range(0, 51)) {
            ...
            if ([2, 5].includes(searchElement)) {
                ...
                if ([1, 4, 8].includes(searchElement)) {
...
async function showFailedTrialFeedback(animationPiano, levels, doneTrialResult, truth) {
    ...
    if (doneTrialResult.hasAccuracyMistakes()) // #0 : #3 (incl)
    {
        ...
        $bigMessage.text("Not perfect, but you got the notes right!");
    } // #4
