...
let {
	sum, concurrent, $fadeOut, $fadeIn, int, wait, num2eng, strong, range, $fadeOutMany, $fadeInMany, bool, Alert
...
class Video {
	...
	constructor($video) {
		...
		// let trimmedFilePath = `${EStore.truthFilePath()}_trimmed.mp4`;
		...
		let data = fs.readFileSync(`${EStore.truthFilePath()}_onsets.json`, { encoding: 'utf8' });
		data = JSON.parse(data);
		this.firstOnset = data.onsets[data.first_onset_index];
		this.vidElm.currentTime = this.firstOnset;
		this.loaded = false;
		// this.vidElm.oncanplaythrough = cpt => console.log('canplaythrough', cpt.valueOf());
		// this.vidElm.ontimeupdate = tu => console.log('tu: ', tu);
	}

	/*_promiseClick() {
		console.log('_promiseClick()');
		Alert.big.blocking({ title: 'Ready when you are' });
		return new Promise(resolve => document.body.addEventListener("click", resolve, { once: true }));
	}
	*/
	...
	get vidElm() {return this.$element[0];}
	...
	async play(numOfNotes = null) {
		...
		if (numOfNotes != null) {
			let messages = await Python.runAsync('InsideTest/normalize_txt_file.py', {
				args: [EStore.truthFilePath('txt')],
				mode: "json"
			});
			let tdeltas = messages.slice(0, numOfNotes).map(msg => msg.time_delta - 0.01);
			if (numOfNotes >= messages.length) {
			...
			let vidDuration = sum(tdeltas.slice(0, numOfNotes + 1));
			console.log({ vidDuration, tdeltas, messages });
			debugger;
		}
		...
		try {
			await concurrent(this.vidElm.play(), wait(this.vidElm.duration * 1000 - this.firstOnset * 1000));
		} catch (e) {
