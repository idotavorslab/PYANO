...
let { strong, bold } = require("pyano_local_modules/util");
 ...
 * @return {Promise<void>}
 */
async function _$messagesInAndOut({ includeSecondary = true, fadeInBigMsg = true, fadeInSmallMsg = true, long = false }) {
...
/**
 * Called by insidetest.js after awaiting playLevelIntroDemo
...
async function $smallMessageAndGuiIn(level) {
	...
	if (level.rhythm) {
		...
		_smallmsg += `${bold('Remember')}: just get the notes right. Don't think about speed or rhythm`;
	}
...
/**@param {Piano} animationPiano
 ...
 @param {Truth} truth
 @return {Promise<void>}*/
async function playLevelIntroDemo({ animationPiano, levels, truth }) {
	console.log(...logmod('playLevelIntroDemo(animationPiano, levels, truth)', { b: true }));


	const currentLevel = levels.current;
	console.log(...log(`\tcurrentLevel`), currentLevel);
	let shouldAlsoRunVideo = EStore.isDemoVideo();
...
async function showFailedTrialFeedback(animationPiano, levels, doneTrialResult, truth) {
	...
	const _hasMistakes = doneTrialResult.hasMistakes();
	if (!_hasMistakes)
		$bigMessage.text("You got the important parts right!");
	else
		$bigMessage.text("Oi vei...").addClass('lightpink');
	await _$bigMessageInGuiOut();
	let _msgsInOutOpts = { fadeInBigMsg: false, includeSecondary: false, long: true };
	if (doneTrialResult.advance_trial) {
		let _smallmsg = `We'll advance to the next trial, but`;
		...
		if (_hasMistakes) {
			...
			_msgsInOutOpts.includeSecondary = true;
		}
		...
		let _smallsec = "Please repeat this trial. The animation will play, showing the notes you got wrong";
		if (doneTrialResult.isTempoBad())
			_smallsec += `<br>Additionally, you played too ${doneTrialResult.tempo_str}`;
...
async function showPassedTrialFeedback(animationPiano, levels, doneTrialResult, truth) {
	...
	if (!isLastTrial || !levels.isCurrentLastLevel()) {
		let _small = `Let's move on to the next `;
		if (isLastTrial)
			_small += `level`;
		else
			_small += `trial`;
		$smallMessage.html(_small);
		await _$messagesInAndOut({ includeSecondary: false, fadeInBigMsg: false });
	}
...
let $pageSubtitle = $('<div id="page_subtitle">')
	.append(
...
let $smallMessage = $('<div id="small_message" class="subtitle" >').hide();
let $smallMessageSecondary = $('<div id="small_message_secondary" class="subtitle">')
	.hide();
