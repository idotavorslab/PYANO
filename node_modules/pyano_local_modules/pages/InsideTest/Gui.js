 ...
 * @return {Promise<void>}
 */
async function _$messagesInAndOut({ includeSecondary = true, fadeInBigMsg = true, long = false }) {
	...
	await asx.$fadeOutMany(1500, ...$messages);
 ...
 * @return {Promise<void>}
 */
async function $smallMessageAndGuiIn(level) {
	console.log(...logmod('\t$smallMessageAndGuiIn()'));
	const _msgs = [`When you're ready, please play <b>${level.notes}</b> notes.<br>`];
	if (level.rhythm) {
		_msgs.push(`Remember to keep rhythm`);
		if (level.tempo == 100)
			_msgs.push(" and regular speed.");
		else
			_msgs.push(`, but don't play any slower than ${level.tempo}% speed.`);

	} else {
		_msgs.push(`Remember: just get the notes right. Don't think about speed or rhythm.`);
	}
	$smallMessage.html(_msgs.join(''));
...
async function _runVideo(numOfNotes, playbackRate = 1) {
	...
	if (playbackRate > 1)
		throw new Error(`_runVideo got playbackRate that's not a float: ${playbackRate}`);
...
async function _runAnimation(playOptions) {
	...
	if (bool(playOptions) && playOptions.playbackRate && playOptions.playbackRate > 1)
		throw new Error(`_runAnimation got playOptions.playbackRate that's not a float: ${playOptions.playbackRate}`);
...
async function playPreTrialDemo({ animationPiano, levels, truth }) {
	...
	if (isFirstTrial)
		...
		$bigMessage.text(`Let's move on to the ${currentLevel.isLastTrial() ? 'last' : currentLevel.internalTrialIndex.human()} trial of the ${currentLevel.index.human()} level.`);
	if (isFirstTrial)
		$smallMessage.html(`You'll now play <b>${currentLevel.notes}</b> notes.`);
	else
		$smallMessage.html(`Again, you'll play <b>${currentLevel.notes}</b> notes.`);
	let playbackRate = levels.getNextTempoOfThisNotes() / 100;
	console.log(...log(`\tshouldAlsoRunVideo = ${shouldAlsoRunVideo}`, { sm: true }));
	const _smallsec = [`Here's ${shouldAlsoRunVideo ? "a video" : "an animation"} `,
	                   `showing only these <b>${currentLevel.notes}</b> notes `,
	                   `at <b>${playbackRate * 100}%</b> speed.<br>`];
	if (currentLevel.rhythm) {
		_smallsec.push(`<b>Try to keep rhythm`);
		if (currentLevel.tempo == 100)
			_smallsec.push(" and regular speed.</b>");
		else
			_smallsec.push(`. Don't play any slower than the following demo.</b>`);

	} else {
		_smallsec.push('<b>You can play as slow as you like.</b>');
	}
	$smallMessageSecondary.html(_smallsec.join(''));
	await _$messagesInAndOut({ includeSecondary: true, long: true });
...
async function showFailedTrialFeedback(animationPiano, levels, doneTrialResult, truth) {
	...
	const { mistakes, advance_trial, tempo_str } = doneTrialResult;
	if (advance_trial) {
		let _small = [`We'll advance to the next trial, but`];
		let _mistakeKind = null;
		if ('accuracy'.in(mistakes) && 'rhythm'.in(mistakes))
			_mistakeKind = 'accuracy and rhythm';
		else if ('accuracy'.in(mistakes))
			_mistakeKind = 'accuracy';
		else if ('rhythm'.in(mistakes))
			_mistakeKind = 'rhythm';
		if (_mistakeKind != null)
			_small.push(` you had some ${_mistakeKind} mistakes`);

		if (tempo_str && tempo_str != 'ok') {
			if (_mistakeKind != null)
				_small.push(`, and you played too ${tempo_str}`);
			else
				_small.push(` you played too ${tempo_str}`);
		}
		$smallMessage.text(_small.join(''));
		// const _smallsec = ["Here's the animation again, showing any notes you got wrong."];
		// if (tempo_str && tempo_str != 'ok')
		// 	_smallsec.push(`<br>Additionally, you played too ${tempo_str}.`);
		...
		const _smallsec = ["Please repeat this trial. The animation will play, showing the notes you got wrong."];
		if (tempo_str && tempo_str != 'ok')
			_smallsec.push(`<br>Additionally, you played too ${tempo_str}.`);
	...
	await _$messagesInAndOut({ fadeInBigMsg: false, long: true });
...
async function showPassedTrialFeedback(animationPiano, levels, doneTrialResult, truth) {
	...
	const currentLevel = levels.current;
	if (currentLevel.isLastTrial())
		$bigMessage.text(`Way to go! You completed the ${currentLevel.index.human()} level.`);
	else
		$bigMessage.text(`Good job! You passed the ${currentLevel.internalTrialIndex.human()} trial of the ${currentLevel.index.human()} level.`);
	await _$bigMessageInGuiOut();
