const { elem } = require("pyano_local_modules/util/Elem");
...
class Animation {
	...
	constructor(animation) {
		...
		for (const i of range(0, 51)) {
			if (i == 0) {
				this.svgs.push(elem({ htmlElement: Svg.blackRight }));
				continue;
			} else if (i == 51) {
				this.svgs.push(elem({ htmlElement: Svg.base }));
				continue;
			}
			...
			if ([2, 5].includes(searchElement))
				...
				this.svgs.push(elem({ htmlElement: Svg.blackLeft }));
			else // [0, 3, 6, 7]
				this.svgs.push(elem({ htmlElement: Svg.blackBoth }));
		}
		this.element.append(...this.svgs);
...
const animationElem = elem({ tag: 'div' }).css({ display: 'none' });
