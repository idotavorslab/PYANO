...
async function playPreTrialDemo({ animationPiano, levels, truth }) {
	...
	if (isFirstTrial)
		...
		$bigMessage.text(`Let's move on to the ${currentLevel.internalTrialIndex.human(true)} trial of the ${currentLevel.index.human(true)} level.`);
	if (isFirstTrial)
	...
	const _smallsec = [`Here's ${shouldAlsoRunVideo ? "a video" : "an animation"}`,
	                   ...
	                   `<b>${currentLevel.rhythm ? 'Try to keep rhythm.' : 'You can play as slow as you like.'}</b>`];
...
/**@param {Piano} animationPiano
 ...
 @param {Truth} truth
 @return {Promise<void>}*/
async function showFailedTrialFeedback(animationPiano, currentLevel, doneTrialResult, truth) {
	console.log(...logmod('showFailedTrialFeedback(animationPiano, currentLevel, doneTrialResult, truth)', { b: true }));
	$bigMessage.text("Oi vei!");
	await _$bigMessageInGuiOut();
	const { mistakes, played_enough_notes, is_tempo_correct } = doneTrialResult;
	const badAccuracyInRhythm = bool(mistakes)
	                            && 'accuracy'.in(mistakes)
	                            && currentLevel.rhythm;
	if (badAccuracyInRhythm) {
		$smallMessage.text(`You got some notes wrong`);
		$smallMessageSecondary.text("Please try this trial again.");
	} else {
		$smallMessage.text(`You almost got the ${currentLevel.internalTrialIndex.human()} trial right.`);
...
/**@param {Piano} animationPiano
 @param {Level} currentLevel
 @param {Truth} truth*/
async function showPassedTrialFeedback(animationPiano, currentLevel, truth) {
	console.log(...logmod('showPassedTrialFeedback(animationPiano, currentLevel, truth)', { b: true }));
	$bigMessage.text(`Good job! You passed the ${currentLevel.internalTrialIndex.human()} trial.`);
	await _$bigMessageInGuiOut();

	const numOfNotes = currentLevel.notes;
	$smallMessage.html(`Let's show you those same <b>${numOfNotes}</b> notes you just got correctly to consolidate the memory.`);
