...
const log = s => [`🎨 %c${s}`, 'color:#a147ad'];
const logmod = s => [`🎨 Gui.%c${s}`, 'color:#a147ad'];
...
class Video {
	...
	async _getWaitValue(numOfNotes) {
		...
		console.log(`_getWaitValue(numOfNotes=`, numOfNotes, ')->', {
			waitValue,
	...
	async play(numOfNotes) {
		...
		this.vidElm.playbackRate = 1;
		let waitValue = await this._getWaitValue(numOfNotes);
		await this._fakeMousedown();
		console.log(...log(`concurrent play, promiseFadeIn, waitValue = ${waitValue}`));
...
/**@param {number} numOfNotes*/
async function _runVideo(numOfNotes) {
...
async function playPreTrialDemo(options) {
	...
	const shouldAlsoRunVideo = EStore.isDemoVideo() && trialIndex == 0 && !EStore.isCheckingRhythmInLevel();
	if (trialIndex == 0)
