const { elem, div, Elem } = require("pyano_local_modules/util/Elem");
...
class Animation {
	...
	constructor(animation) {
		...
		for (const i of range(0, 51)) {
			if (i == 0) {
				...
				whitekey.cacheAppend({ right: div({ cls: 'black-key right' }) });
				whitekey.blacks = () => [whitekey.right];
			...
			if ([2, 5].includes(searchElement)) {
				...
				let whitekey = div({ cls: 'white-key black-single-side' });
				whitekey.cacheAppend({ left: div({ cls: 'black-key left' }) });
				whitekey.blacks = () => [whitekey.left];
				this.whitekeys.push(whitekey);
			} else {// [0, 3, 6, 7]
				let whitekey = div({ cls: 'white-key black-both-sides' });
				whitekey.cacheAppend({ left: div({ cls: 'black-key left' }) });
				whitekey.cacheAppend({ right: div({ cls: 'black-key right' }) });
				whitekey.blacks = () => [whitekey.left, whitekey.right];
				this.whitekeys.push(whitekey);
			}
		}
		this.element.append(...this.whitekeys);
	}
	...
	async markKey(note, noteDuration, fill) {
		return new Promise(async resolve => {
			...
			if (index.length != undefined) { // [33,32] (D, E) for example
				const [indexLeft, indexRight] = index;
				const $keyLeft = this.whitekeys[indexRight - 1].left;
				const $keyRight = this.whitekeys[indexLeft - 1].right;
				toMark = [$keyLeft, $keyRight];
				baseFill = 'black';
			} else { // 33 (E)
				toMark = [this.whitekeys[index - 1]];
				baseFill = 'white';
			}
			toMark.forEach(rect => rect.css({ backgroundColor: fill }));
			await asx.wait(noteDuration);
			toMark.forEach(rect => rect.css({ backgroundColor: baseFill }));
