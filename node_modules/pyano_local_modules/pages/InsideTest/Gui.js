...
async function _show$bigMessageHideGreenPrompt() {
	...
	await asx.concurrent(
		asx.$fadeInMany(700, $bigMessage, $skipButton),
...
async function show$smallMessageAndGreenPrompt(level) {
	console.log(...logmod('\tshow$smallMessageAndGreenPrompt()'));
...
async function _runVideo(numOfNotes, playbackRate = 1) {
	...
	if (playbackRate > 3)
		throw new Error(`_runVideo got playbackRate > 3: ${playbackRate}`);
...
async function _runAnimation(playOptions) {
	...
	if (bool(playOptions) && playOptions.playbackRate && playOptions.playbackRate > 3)
		throw new Error(`_runAnimation got playOptions.playbackRate > 3: ${playOptions.playbackRate}`);
...
/**@param {Piano} animationPiano
 ...
 @param foo
 @return {Promise<void>}*/
function showFailedTrialFeedback(animationPiano, levels, doneTrialResult, truth, foo) {
	return new Promise(async (resolve, reject) => {
			console.log(...logmod('showFailedTrialFeedback(animationPiano, levels, doneTrialResult, truth)', { b: true }));
			const lg = (s, bold = false) => [`%c${s}`, `color: #dec56c; ${bold ? 'font-weight: 900' : ''}`];
			let cancelled = false;
			foo.cancel = () => {
				console.log(...lg('setting cancelled=true', true));
				cancelled = true;
				return reject(new Error("foo.cancel=() return reject"));
			};
			...
			console.log(...lg('before _show$bigMessageHideGreenPrompt'));
			await wrapWithCancel(_show$bigMessageHideGreenPrompt)();
			console.log(...lg('after _show$bigMessageHideGreenPrompt'));
			if (cancelled) {
				return reject(new Error('after _show$bigMessageHideGreenPrompt SKIP'));
			}
			...
			console.log(...lg('before _$messagesInAndOut'));
			await wrapWithCancel(_$messagesInAndOut)(_msgsInOutOpts);
			console.log(...lg('after _$messagesInAndOut'));
			if (cancelled) {
				return reject(new Error('after _$messagesInAndOut SKIP'));
			}
			...
			if (cancelled) {
				return reject(new Error('before _runAnimation SKIP'));
			}
			console.log(...lg('before _runAnimation'));
			return await wrapWithCancel(_runAnimation)(playOptions);

			/**@param {function} fn*/
			function wrapWithCancel(fn) {
				console.log(...lg(`wrapping fn: ${fn.name}`));
				return fnargs => {
					if (!cancelled) {
						console.log(...lg(`wrapWithCancel (${fn.name}) (cancelled=${cancelled}), calling`));
						const res = fn(fnargs);
						console.log(...lg(`wrapWithCancel ${fn.name} returned (${typeof res}): ${res}`));
						return res;
					} else {
						console.log(...lg(`wrapWithCancel (${fn.name}) (cancelled=${cancelled}), returning from wrapper`));
					}
				};
			}
...
async function showTestCompleteMessages() {
	...
	// $smallMessageSecondary.html('See you soon');
	await asx.concurrent(
		asx.$fadeOut($skipButton, 150),
