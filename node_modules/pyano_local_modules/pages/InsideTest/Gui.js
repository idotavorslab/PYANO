const { elem, div } = require("pyano_local_modules/util/Elem");

const { strong, span } = require("pyano_local_modules/util");
const { EStore, TL } = require("pyano_local_modules/ext_libs");
...
class Animation {
	...
	constructor(animation) {
		...
		for (const i of range(0, 51)) {
			if (i == 0) {
				let whitekey = div({ cls: 'white-key black-single-side' });
				whitekey.append(div({ cls: 'black-key right' }));
				this.pianokeys.push(whitekey);
				continue;
			} else if (i == 51) {
				let whitekey = div({ cls: 'white-key black-single-side' });
				this.pianokeys.push(whitekey);
			...
			if ([2, 5].includes(searchElement)) {
				...
				let whitekey = div({ cls: 'white-key black-single-side' });
				whitekey.append(div({ cls: 'black-key left' }));
				this.pianokeys.push(whitekey);
			} else {// [0, 3, 6, 7]
				let whitekey = div({ cls: 'white-key black-both-sides' });
				whitekey.append(div({ cls: 'black-key left' }));
				whitekey.append(div({ cls: 'black-key right' }));
				this.pianokeys.push(whitekey);
			}
		}
		this.element.append(...this.pianokeys);
	}

	_baseSvg() {
		const base = document.createElement('SVG');
		base.setAttribute('xmlns', "http://www.w3.org/2000/svg");
		base.setAttribute('xmlns:xlink', "http://www.w3.org/1999/xlink");
		base.setAttribute('viewBox', "0 0 1 1");

		const baseRect = document.createElement('RECT');
		baseRect.classList.add('piano-white-rect');
		base.appendChild(baseRect);
		return base;
	}

	_blackRightSvg() {
		const blackRight = document.createElement('SVG');
		blackRight.setAttribute('xmlns', "http://www.w3.org/2000/svg");
		blackRight.setAttribute('xmlns:xlink', "http://www.w3.org/1999/xlink");
		blackRight.setAttribute('viewBox', "0 0 1 1");
		const blackRightRect1 = document.createElement('RECT');
		blackRightRect1.classList.add('piano-white-rect');
		const blackRightRect2 = document.createElement('RECT');
		blackRightRect2.classList.add('piano-black', 'piano-black-right-rect');
		blackRight.appendChild(blackRightRect1);
		blackRight.appendChild(blackRightRect2);
		return blackRight;
	}

	_blackLeftSvg() {
		const blackLeft = document.createElement('SVG');
		blackLeft.setAttribute('xmlns', "http://www.w3.org/2000/svg");
		blackLeft.setAttribute('xmlns:xlink', "http://www.w3.org/1999/xlink");
		blackLeft.setAttribute('viewBox', "0 0 1 1");
		const blackLeftRect1 = document.createElement('RECT');
		blackLeftRect1.classList.add('piano-white-rect');
		const blackLeftRect2 = document.createElement('RECT');
		blackLeftRect2.classList.add('piano-black', 'piano-black-left-rect');
		blackLeft.appendChild(blackLeftRect1);
		blackLeft.appendChild(blackLeftRect2);
		return blackLeft;
	}

	_blackBothSvg() {
		const blackBoth = document.createElement('SVG');
		blackBoth.setAttribute('xmlns', "http://www.w3.org/2000/svg");
		blackBoth.setAttribute('xmlns:xlink', "http://www.w3.org/1999/xlink");
		blackBoth.setAttribute('viewBox', "0 0 1 1");
		const blackBothRect1 = document.createElement('RECT');
		blackBothRect1.classList.add('piano-white-rect');
		const blackBothRect2 = document.createElement('RECT');
		blackBothRect2.classList.add('piano-black', 'piano-black-left-rect');
		const blackBothRect3 = document.createElement('RECT');
		blackBothRect3.classList.add('piano-black', 'piano-black-right-rect');
		blackBoth.appendChild(blackBothRect1);
		blackBoth.appendChild(blackBothRect2);
		blackBoth.appendChild(blackBothRect3);
		return blackBoth;
	}
	...
	async markKey(note, noteDuration, fill) {
		return new Promise(async resolve => {
			...
			let toMark = undefined;
			let baseFill = undefined;
			// const children = this.element.children();
			if (index.length != undefined) { // [3,4] for example
				const [indexLeft, indexRight] = index;
				const $keyLeft = this.pianokeys[indexRight].child(`.piano-black-left-rect`);
				const $keyRight = this.pianokeys[indexLeft].child(`.piano-black-right-rect`);
				toMark = [$keyLeft, $keyRight];
				baseFill = 'black';
			} else { // 3
				toMark = [this.pianokeys[index].child(`.piano-white-rect`)];
				baseFill = 'white';
			}
			toMark.forEach(rect => rect.css({ fill }));
			await asx.wait(noteDuration);
			toMark.forEach(rect => rect.css({ fill: baseFill }));
...
const animationElem = elem({ tag: 'div' }).id('animation_nongrid');
const animation = new Animation(animationElem);
