...
function checkDoneTrial(truth, trialTruth) {
	console.log(...log('checkDoneTrial(truth, trialTruth)'));
	return new Promise((resolve, reject) => {

		/*const full_truth_file_path = EStore.truthFilePath('txt');
		const trial_file_path = EStore.trialFilePath('txt');
		*/
		const { allowed_tempo_deviation_factor } = EStore.currentTest();
		const current_level = EStore.getCurrentLevel();
		// let num_of_notes_to_test = EStore.numOfNotesCurrLevel();
		...
		*/
		const args = [allowed_tempo_deviation_factor, trialTruth.txt.on.path,
		              truth.txt.on.path, JSON.stringify(current_level)];
		console.log(...log('\tcheckDoneTrial, sending args in json mode: '), args);
