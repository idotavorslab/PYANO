...
async function _runTrial(userMidi, animationPiano, truth) {
	console.log(...log('_runTrial(userMidi, animationPiano, truth)'));
	Gui.updateLevelTrialSubtitles(EStore.currentTest().levels, truth);
	...
	*/
	...
	await Gui.$smallMessageAndGuiIn(levels.current.notes);
...
async function _handleDoneTrial(userMidi, animationPiano, truth, trialTruth) {
	...
	const dontAdvanceTrial = bool(doneTrialResult.mistakes)
	                         && 'accuracy'.in(doneTrialResult.mistakes)
	                         && currentLevel.rhythm;
	console.log(...log(`\tdontAdvanceTrial: `), dontAdvanceTrial);
	if (doneTrialResult.passed) {
		await Gui.showPassedTrialFeedback(animationPiano, currentLevel, truth);
	} else { // did not pass

		await Gui.showFailedTrialFeedback(animationPiano, currentLevel, doneTrialResult, truth);
	}
