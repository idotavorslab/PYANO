...
async function _maybeCreateOnsetsJson(truthPathNoExt) {
	...
	if (!exists) {
		...
		let data = JSON.parse(fs.readFileSync(onsetsJsonPath));
		let datakeys = Object.keys(data);
		if (!("onsets".in(datakeys) && "first_onset_index".in(datakeys))) {
			let { value } = await Alert.big.blocking({
...
const insideTestPage = {
	switch: async (reload) => {
		console.log(...log('insideTestPage.switch()'));
		...
		let midi = new Midi({ muteUserPiano: true });
		...
		console.log(...log('AFTER _maybeCreateOnsetsJson'));
		$MainContent.empty();
