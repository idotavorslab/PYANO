...
const insideTestPage = {
	switch: async reload => {
		...
		/**@type {Piano}*/
		const animationPiano = new Piano(`file:///${EStore.salamanderDirPath()}`, 'insidetest.animationPiano').toMaster();
		...
		Gui.$bigButton
		   .click(async () => {
			   ...
			   let doneTrialResult;
			   try {
				   doneTrialResult = await _handleDoneTrial(userMidi, animationPiano, truth, trialTruth);
			   } catch (e) {
				   console.error(e);
				   await Alert.big.blocking({
					   title: `Couldn't handle done trial. Please screenshot and save.`,
					   html: e.message,
					   type: "error",
					   timer: null,
