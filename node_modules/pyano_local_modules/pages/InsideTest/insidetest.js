...
let {
	$fadeIn, $fadeOut, $fadeOutMany,
	concurrent, date2datetime, mkdir, path_exists, reloadPage,
...
async function _runTrial(midi, playbackPiano) {
	...
	if (EStore.isLearningTempo()) {
		numOfNotes = EStore.truth().numOfNotes();
	} else {
...
async function _handleDoneTrial(midi, playbackPiano, trialTruth) {
	...
	let msgs = await Python.runAsync('InsideTest/merge_on_off_txt_files.py', {
		args: [trialTruth.txt.base, trialTruth.txt.on, trialTruth.txt.off],
		mode: "json"
	});
	...
	if (passed) {
		await Gui.showPassedTrialFeedback(midi, playbackPiano, trialIndex, trialTruth);
	} else { // did not pass

		await Gui.showFailedTrialFeedback({ midi, playbackPiano, trialIndex, mistakes, trialTruth });
	}
	...
	if (!badAccuracyInTempo) {
		...
		fs.renameSync(`${trialTruth.pathNoExt}.txt`, `${newPath}.txt`);
		fs.renameSync(`${trialTruth.pathNoExt}.mid`, `${newPath}.mid`);
	}
}
...
const insideTestPage = {
	switch: async reload => {
		...
		let trialTruth = await _runTrial(midi, playbackPiano);
		...
		Gui.$bigButton
		   .click(async () => {
			   ...
			   await _handleDoneTrial(midi, playbackPiano, trialTruth);
