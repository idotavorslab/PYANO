...
async function _runTrial({ userMidi, animationPiano, truth, doneTrialResult = undefined }) {
	console.log(...log(`_runTrial(userMidi, animationPiano, truth, doneTrialResult = `, { b: true }), doneTrialResult);
	const levels = EStore.getLevels();
	Gui.updateLevelTrialSubtitles(levels, truth);
	let playLevelIntroDemo;
	if (EStore.dev.skip_level_intro_demo()) {
		playLevelIntroDemo = false;
		console.log(...log('skip_level_intro_demo', { sm: true }));
	} else {
		if (doneTrialResult && doneTrialResult.advance_trial === false)
			playLevelIntroDemo = false;
		else
			playLevelIntroDemo = levels.current.isFirstTrial();
	}

	if (playLevelIntroDemo)
		await Gui.playLevelIntroDemo({ animationPiano, levels, truth });
...
const insideTestPage = {
	switch: async reload => {
		...
		EStore.set({ last_page: 'inside_test', 'current_test.finished_trials_count': 0 });
		if (reload)
			return reloadPage();
		...
		$Title.text(`testing ${EStore.subjectName()}`.title());
		const levels = EStore.getLevels();
		Gui.updateLevelTrialSubtitles(levels, truth);
		await asx.$fadeIn($MainContent, 300);
		console.groupEnd();

		if (!EStore.dev.skip_whole_truth_demo())
			await Gui.playWholeTruthDemo(animationPiano, truth, levels.maxNotes());
		else
			console.log(...log('skip_whole_truth_demo', { sm: true }));
