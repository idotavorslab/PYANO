	...
	if (doneTrialResult.passed && !EStore.dev.skip_passed_trial_feedback()) {
		...
		try {
			...
			$skipButton[0].addEventListener('click', () => {
				console.log('click, setting shouldSkip = true. promise: ', promise);
				shouldSkip = true;
				console.log('\tBEFORE foo.cancel()');
				foo.cancel();
				console.log('\tAFTER foo.cancel()');
				throw new Error("click new error");


			});
			const promise = Gui.showFailedTrialFeedback(animationPiano, levels, doneTrialResult, truth, foo);

			if (!shouldSkip) {
				console.log(...log('awaiting showFailed promise'));
				await Promise.resolve(promise);
			} else {
				console.log(...log('skipped failed feedback'));
			}
		} catch (e) {
			console.error(e);
		}
