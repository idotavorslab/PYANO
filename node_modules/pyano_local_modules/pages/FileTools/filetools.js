...
async function split_base_txt_file(truth) {
	console.log(...log('split_base_txt_file(truth)'));
	await Python.runAsync('FileTools/split_base_txt_file.py', {
...
async function txt2midi(truth) {
	...
	if (!(await truth.txt.allExist())) {
		let confirm = await Alert.big.blocking({
			title: `There seem to be some txt files missing (on file, off file or both)`,
			html: `Do you wish to create them from the base txt file?`,
			showConfirmButton: true, showCancelButton: true
		});
		if (!confirm.value)
			return Alert.small.info('Aborting.');
		try {
			await split_base_txt_file(truth);
		} catch (e) {
			console.error(e);
			Alert.small.error(`Couldn't split base file to on and off txt files`, e.message);
		}
	}
	let on_off_pairs;
	try {
		on_off_pairs = (await PyFns().merge_on_off_txt_files(truth)).on_off_pairs;
	} catch (e) {
		console.error(e);
		Alert.small.error(`Couldn't merge on and off txt files`, e.message);
	}
