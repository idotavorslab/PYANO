...
let { push_before_ext, int, reloadPage, replace_ext, round, path_exists, $fadeOut, $fadeIn } = require("pyano_local_modules/util");
...
async function handleMov(movname, movpath) {
	...
	try {
		...
		let cmd = `ffmpeg ${yesflag} -i "${movpath}" -filter:v "crop=1900:400:10:360" -qp 11 -preset ultrafast -tune zerolatency -profile:v baseline -level 3.0 -movflags +faststart "${mp4path}"`;
