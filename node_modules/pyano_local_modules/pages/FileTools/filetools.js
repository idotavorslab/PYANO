...
let { push_before_ext, date2datetime, int, reloadPage, replace_ext, round, path_exists, $fadeOut, $fadeIn } = require("pyano_local_modules/util");
...
async function handleTxt(txtname, txtpath) {
	...
	try {
		...
		Alert.small.success(`Created ${replace_ext(txtname, 'mid')}.`);
	} catch (e) {
...
async function handleMid(midiname, midipath) {
	const txtname = replace_ext(midiname, 'txt');
	let shouldConvert = await Alert.big.blocking({
		title: `Convert ${midiname} to ${txtname}?`
	});
	if (!shouldConvert.value) {
		return Alert.small.info('Not converting');
	}
	const txtpath = replace_ext(midipath, 'txt');
	if (await path_exists(txtpath)) {
		return Alert.small.warning('Txt file already exists. Aborting.');
	}
	let { toTxtFromMidi } = require("pyano_local_modules/gilad");
	try {
		await toTxtFromMidi(midipath);
		// let messages = await Python.runAsync('InsideTest/normalize_txt_file.py', {
		// 	args: [txtpath],
		// 	mode: "json"
		// });
		// console.log({ messages });
		Alert.small.success(`Created ${txtname}.`);
	} catch (e) {
		Alert.small.error(`Converting ${midiname} to ${txtname} failed`, e);
	}
...
async function handleMov(movname, movpath) {
	...
	let mp4path = replace_ext(movpath, 'mp4');
...
async function handleMp4(mp4name, mp4path) {
	...
	if (!compressed || !cropped) {
		const movpath = replace_ext(mp4path, 'mov');
		if (await path_exists(movpath)) {
