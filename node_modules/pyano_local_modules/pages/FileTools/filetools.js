...
async function handleTxt(txtname, txtpath) {
	...
	let mid = fsx.replace_ext(txtpath, 'mid');
	if (await fsx.path_exists(mid)) {
		const midstats = await fs.statSync(mid);
		const datestr = date2datetime(midstats.birthtime);
		const midnew = fsx.push_before_ext(mid, `_${datestr}`);
	...
	try {
		...
		Alert.small.success(`Created ${fsx.replace_ext(txtname, 'mid')}.`);
	} catch (e) {
...
async function handleMid(midiname, midipath) {
	...
	const txtPathNoExt = fsx.remove_ext(midipath);
	const txtPathOn = `${txtPathNoExt}_on.txt`;
	const txtPathOff = `${txtPathNoExt}_off.txt`;
	const txtFilePath = `${txtPathNoExt}.txt`;
	let [onExists, offExists, baseExists] = await asx.concurrent(
		fsx.path_exists(txtPathOn),
		fsx.path_exists(txtPathOff),
		fsx.path_exists(txtFilePath));
	...
	try {
		let [txtFilePath, txtPathOn, txtPathOff] = await toTxtFromMidi(midipath);
		let on_messages = await Python.runAsync('InsideTest/normalize_txt_file.py', {
			args: [txtPathOn],
			mode: "json"
		});
		console.log({ on_messages });
		await Python.runAsync('InsideTest/merge_on_off_txt_files.py', {
			args: [txtFilePath, txtPathOn, txtPathOff],
			mode: "json"
		});
