...
let { reloadPage, replace_to_ext, round, path_exists, $fadeOut, $fadeIn } = require("pyano_local_modules/util");
...
async function handleMp4(mp4name, mp4path) {
	if ('trimmed'.in(mp4name)) {
		return Alert.small.error(`Don't use trimmed videos.`, `Drop the original file instead`);
	} else {
		await writeFullOnsetsJson(mp4name, mp4path);
	}
