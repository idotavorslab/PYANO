...
class MyStore extends Store {
    ...
    get experiment_type() {
        let experimentType = this.get('experiment_type');
        console.debug(`MyStore.experiment_type() → ${experimentType}`);
    ...
    get root_abs_path() {
        ...
        console.debug('MyStore.root_abs_path() →', root_abs_path);
    ...
    fromSavedConfig(savedConfig, experimentType) {
        console.debug(`MyStore.fromSavedConfig(experimentType='${experimentType}', savedConfig=${JSON.stringify(savedConfig)})`);
    ...
    config(type = undefined) {
        ...
        if (type) {
            ...
            config = new Config(this.experiment_type);
        }
        console.debug(`MyStore.config('${type}') → `, freeze(config));
        return config;
    }
...
class Config extends MyStore {
    ...
    /**@return {string}*/
    get save_path() {
        let save_path = this._get('save_path');
        console.debug(`Config.save_path() → ${save_path}`);
        return save_path;
    }

    /**@param {string} savePath*/
    set save_path(savePath) {
        console.debug(`Config.save_path('${savePath}')`);
    ...
    /**@return {TLevel[]}*/
    get levels() {
        let levels = this._get('levels');
        console.debug(`Config.levels() →`, freeze(levels));
        return levels;
    }
    ...
    set levels(levels) {
        ...
        console.debug('Config.levels(levels:', freeze(levels), ')');
    ...
    fromSavedConfig(savedConfig, ...args) {
        console.debug(`Config.fromSavedConfig(savedConfig:`, freeze(savedConfig), ')');
    ...
    _updateSavedFile(key, value) {
        ...
        console.log('💾 _updateSavedFile(key,value)', freeze({key, value, conf}));
        conf.set(key, value);
    }
    ...
    getLevels() {
        let [level_index, trial_index] = this.currentTrialCoords();
        let levels = new Levels(this.levels, level_index, trial_index);
        console.debug('Config.getLevels() →', levels.toString());
        return levels;
    }
