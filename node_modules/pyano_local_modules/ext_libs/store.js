...
class MyStore extends Store {
	...
	*/
	...
	/**@param {TSavedConfig} config*/
	fromSavedConfig(config) {
		this.experiment_type = config.experiment_type;
		this.config('exam').fromConfig(config.current_exam);
		this.config('test').fromConfig(config.current_test);
	}
	...
	/**@protected
	 * @param {'experiment_type' | TConfigKey} key
	 * @param value*/
	_updateSavedFile(key, value) {
		const conf = new (require('conf'))({
			cwd: Path.dirname(Path.join(this.root_abs_path, this.save_path)),
			configName: Path.basename(this.save_path, '.json')
		});
		conf.set(key, value);
	}
...
class Config extends MyStore {
	...
	toObj() {
		return super.get(`current_${this.type}`);
	}

	/**@param {TConfig} config*/
	fromConfig(config) {
		this.truth_file_path = new Truth(Path.join(super.root_abs_path, fsx.remove_ext(config.truth_file_path)));
		this.levels = config.levels;
		this.finished_trials_count = config.finished_trials_count;
		this.errors_playingspeed = config.errors_playingspeed;
		this.demo_type = config.demo_type;
		this.current_subject = config.current_subject;
		this.allowed_rhythm_deviation = config.allowed_tempo_deviation;
		this.allowed_rhythm_deviation = config.allowed_rhythm_deviation;
	}
	...
	_set(keyOrObj, value) {
		...
		if (type === 'string') {
			const _key = `current_${this.type}.${keyOrObj}`;
			super.set(_key, value);
			super._updateSavedFile(_key, value);
			return;
		}
		if (type === 'object') {
			if (keyOrObj.keys().some(key => !key.in(Config._KEYS)))
				throw new Error(`Config(${this.type}).set: "keyOrObj" is object, some keys not in this._KEYS. keyOrObj keys: ${keyOrObj.keys()
				                                                                                                                       .join(', ')}`);
			for (const key in keyOrObj) {
				let _key = `current_${this.type}.${key}`;
				super.set(_key, keyOrObj[key]);
				super._updateSavedFile(_key, keyOrObj[key]);
			}

			return;

		}
		throw new TypeError(`Config(${this.type}).set: arg "keyOrObj" is not string or object. type: ${type}. keyOrObj: ${keyOrObj}`);
	}
