...
class MyStore extends Store {
	...
	setTruthFilePath(truth) {
		return truth.txt.allExist()
		            .then(exist => {
			            if (exist) {
				            this.set('current_test.truth_file_path', `experiments\\truths\\${truth.txt.base.name}`);
				            return truth;
			            } else {
	...
	/**@return {Truth}*/
	truth() {
		return new Truth(Path.join(this.truthsDirPath(), Path.basename(this.get("current_test.truth_file_path"), '.txt')));
	}
	...
	*/
	...
	/*isCheckingRhythmInLevel() {
		console.warn('EStore.isCheckingRhythmInLevel() was called, should use getCurrentLevel() or getLevels() instead');
		let { levels } = this.currentTest();
		let [level_index, _] = this.currentTrialCoords();
		return levels[level_index].rhythm;
	...
	*/
	...
	/*numOfNotesCurrLevel() {
		console.warn('"numOfNotesCurrLevel()" was called, "getCurrentLevel()" should be used instead');
	...
	*/
	...
	getCurrentLevel() {
		let [level_index, trial_index] = this.currentTrialCoords();
		return new Level(this.currentTest().levels[level_index], level_index, trial_index);
	}

	/**@return {Levels}*/
	getLevels() {
		let [level_index, trial_index] = this.currentTrialCoords();
		return new Levels(this.currentTest().levels, level_index, trial_index);
	}

	/*maxNotesAmongLevels() {
		console.warn('"numOfNotesCurrLevel()" was called, "getLevels().maxNotes()" should be used instead');
		return max(...this.currentTest().levels.map(lvl => lvl.notes));
	}
	*/
