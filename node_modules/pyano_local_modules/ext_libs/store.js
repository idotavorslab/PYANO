...
let { blockingSwalMixin } = require("pyano_local_modules/util/Alert");
...
class MyStore extends Store {
	...
	setTruthFilePath(fileNameNoExt) {
		if (fileNameNoExt.includes('.'))
			throw new Error(`fileNameNoExt included dot. Pass without extension. received: ${fileNameNoExt}`);
		if (Path.basename(fileNameNoExt) != fileNameNoExt)
			throw new Error(`fileNameNoExt is not just a name, it's some path with dirs. Pass only name. received: ${fileNameNoExt}`);
		this.set('current_test.truth_file_path', `experiments\\truths\\${fileNameNoExt}.txt`);
	}


	/**@example
	 update('current_test.finished_trials_count')
	 @param {String} K
	 @param kv
	 @return {*} */
	update(K, kv) {
	...
	currentTrialCoords() {
		...
		let { sum, enumerate } = require("pyano_local_modules/util");
		...
		throw "getCurrentTrialTuple: out of index error";
	}
	...
	truthFileTotalNotes() {
		const fs = require('fs');
		let { bool } = util();
		const truthFilePath = this.truthFilePath('txt');
		try {
			return fs.readFileSync(truthFilePath, { encoding: 'utf8' })
			         .split('\n')
			...
			const truthFiles = this.truthFilesList('txt')
			                       .map(f => $(`<p class="clickable">${f}</p>`))
			                       .map($f => $f.click(async () => {
				                       this.setTruthFilePath(Path.basename($f.text(), '.txt'));
				                       let { remote } = require("electron");
				                       remote.getCurrentWindow().reload();
			                       }));
			console.log({ truthFiles });
			blockingSwalMixin.fire({
