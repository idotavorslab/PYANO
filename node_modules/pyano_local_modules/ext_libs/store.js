...
class MyStore extends Store {
	...
	_doTruthFileCheck() {
		...
		truth.txt.allExist()
		     .then(exist => {
			     if (exist) {
				     return Alert.small.success('Truth files check completed successfully.');
			     } else {
				     ...
				     Alert.big.blocking({
					     ...
					     clickFn: $s =>
						     this.setTruthFilePath(new Truth(fsx.remove_ext($s.text())))
						         .then(() => {
							         this.set({
		...
		return truth.txt.allExist()
		            .then(exist => {
			            if (exist) {
				            ...
				            return new Error(`Not all txt files of truth exist: ${truth.txt.base.name}`);
			            }
		            });
	}
